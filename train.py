...
def main():
    ...
    # saving path
    path = os.path.join('drive/MyDrive/Adversarial_Learning/ckpt', args.dataset, args.network)
    ...
    # fix seed if given one, else random seeds
    if(args.seed != 0):
        np.random.seed(args.seed)
        torch.manual_seed(args.seed)
        torch.cuda.manual_seed(args.seed)

    curModel = args.model_num
    
    print('Starting model %s.' % curModel)
    ...
    if os.path.exists(os.path.join(args.save_dir, 'model_%s_latest.pth' % curModel)):
        pretrained_model = torch.load(os.path.join(args.save_dir, 'model_%s_latest.pth' % curModel), map_location='cpu')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if test_acc > best_clean_acc:
            ...
            torch.save(best_state, os.path.join(args.save_dir, 'model_%s.pth' % curModel))
            torch.save(model.state_dict(), os.path.join(args.save_dir, 'weight_%s_best.pth' % curModel))
        ...
        if args.adv_training:
            ...
            torch.save(best_state, os.path.join(args.save_dir, 'model_%s_latest.pth' % curModel))
            torch.save(model.state_dict(), os.path.join(args.save_dir, 'weight_%s_latest.pth' % curModel))
        else:
            best_state = {}
            best_state['state_dict'] = copy.deepcopy(model.state_dict())
            best_state['opt'] = copy.deepcopy(opt.state_dict())
